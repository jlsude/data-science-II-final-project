...
const LowProbability = () => {
    ...
    useEffect(() => {
        if(finalResultData.SVM_Model_less){
            setFinalProbability((finalResultData.SVM_Model_less.SVM_probability*100).toFixed(2))
            setSVMCM(finalResultData.SVM_Model_less.SVM_confusion_matrix)
            setSVMPrediction(finalResultData.SVM_Model_less.SVM_prediction)
            setSVMAccuracy((finalResultData.SVM_Model_less.SVM_accuracy*100).toFixed(2))
        }

        if(finalResultData.LR_Model_less){
            setLRProbability((finalResultData.LR_Model_less.LR_probability*100).toFixed(2))
            setLRConfusionMatrix(finalResultData.LR_Model_less.LR_confusion_matrix)
            setLRAccuracy((finalResultData.LR_Model_less.LR_accuracy*100).toFixed(2))
        }

        if(finalResultData.KNN_Model_less){
            setKNNProbability((finalResultData.KNN_Model_less.KNN_probability*100).toFixed(2))
            setKNNConfusionMatrix(finalResultData.KNN_Model_less.KNN_confusion_matrix)
            setKNNPrediction(finalResultData.KNN_Model_less.KNN_prediction)
            setKNNAccuracy((finalResultData.KNN_Model_less.KNN_accuracy*100).toFixed(2))
        }
